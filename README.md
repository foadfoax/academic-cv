# Academic CV – Ghafar (Foad) Boudaghi

This repository contains my academic CV for MSc applications in Artificial Intelligence / Computer Science in Italy, with a focus on securing a fully funded opportunity through **DSU scholarships**.

📄 CV (PDF):
Academic_CV_Ghafar_Foad_Boudaghi.pdf

🔗 GitHub Profile:
https://github.com/foadfoax


## Certifications & Online Courses

- **CS50x: Introduction to Computer Science – Harvard University**  
  Comprehensive course covering algorithms, data structures, problem-solving,
  and software development fundamentals. Completed with a final project.

- **Introduction to Prompt Engineering for Generative AI – LinkedIn Learning**  
  Focused on prompt design, AI interaction strategies, and applied generative AI concepts.
